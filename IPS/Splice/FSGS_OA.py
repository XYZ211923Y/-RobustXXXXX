...
class Attacker(object):
    ...
    def attack(self, funccall, y, failure_set):
        ...
        if current_object > 0:
            ...
            return mf_process, greedy_set_process, changed_set_process, iteration, robust_flag, np.float(orig_g), \
                   greedy_set, greedy_set_visit_idx, greedy_set_best_temp_funccall.tolist(), np.float(orig_g), \
                   np.float(current_object), final_changed_set, n_changed, flip_funccall.tolist(), flip_set, \
                   np.float(flip_object), failure_set
        ...
        return mf_process, greedy_set_process, changed_set_process, iteration, robust_flag, np.float(orig_g), \
               greedy_set, greedy_set_visit_idx, greedy_set_best_temp_funccall.tolist(), np.float(g), \
               np.float(current_object), final_changed_set, n_changed, \
...
log_attack = open(
    '../Logs/gene/%s/mindiff_Attack.bak' % Model_Type, 'w+')
...
for i in range(len(X)):
    ...
    for j in range(4):
        ...
        changed_set, num_changed, flip_funccall, flip_set, flip_object, failure_set = \
            attacker.attack(sample, label, failure_set)
    ...
    pickle.dump(mf_process_all,
                open('../Logs/gene/%s/mindiff_mf_process.pickle' % Model_Type, 'wb'))
    pickle.dump(greedy_set_process_all,
                open('../Logs/gene/%s/mindiff_greedy_set_process.pickle' % Model_Type, 'wb'))
    pickle.dump(changed_set_process_all,
                open('../Logs/gene/%s/mindiff_changed_set_process.pickle' % Model_Type, 'wb'))
    pickle.dump(iterations_all,
                open('../Logs/gene/%s/mindiff_iteration.pickle' % Model_Type, 'wb'))
    pickle.dump(robust_flag_all,
                open('../Logs/gene/%s/mindiff_robust_flag.pickle' % Model_Type, 'wb'))
    pickle.dump(orignal_funccalls_all,
                open('../Logs/gene/%s/mindiff_original_funccall.pickle' % Model_Type,
                     'wb'))
    pickle.dump(orignal_labels_all,
                open('../Logs/gene/%s/mindiff_original_label.pickle' % Model_Type,
                     'wb'))
    pickle.dump(orignal_g_all,
                open('../Logs/gene/%s/mindiff_original_g.pickle' % Model_Type, 'wb'))
    pickle.dump(final_greedy_set_all,
                open('../Logs/gene/%s/mindiff_greedy_set.pickle' % Model_Type, 'wb'))
    pickle.dump(final_greedy_set_visit_idx_all,
                open('../Logs/gene/%s/mindiff_feature_greedy_set.pickle' % Model_Type,
                     'wb'))
    pickle.dump(final_funccall_all,
                open('../Logs/gene/%s/mindiff_modified_funccall.pickle' % Model_Type,
                     'wb'))
    pickle.dump(final_mf_all,
                open('../Logs/gene/%s/mindiff_final_mf.pickle' % Model_Type, 'wb'))
    pickle.dump(final_changed_set_all,
                open('../Logs/gene/%s/mindiff_changed_set.pickle' % Model_Type, 'wb'))
    pickle.dump(final_changed_num_all,
                open('../Logs/gene/%s/mindiff_changed_num.pickle' % Model_Type, 'wb'))
    pickle.dump(flip_funccall_all,
                open('../Logs/gene/%s/mindiff_flip_funccall.pickle' % Model_Type,
                     'wb'))
    pickle.dump(flip_set_all,
                open('../Logs/gene/%s/mindiff_flip_set.pickle' % Model_Type, 'wb'))
    pickle.dump(flip_sample_original_label_all,
                open('../Logs/gene/%s/mindiff_flip_sample_original_label.pickle' % Model_Type, 'wb'))
    pickle.dump(flip_sample_index_all,
                open('../Logs/gene/%s/mindiff_flip_sample_index.pickle' % Model_Type, 'wb'))
    pickle.dump(failue_set_all,
                open('../Logs/gene/%s/mindiff_failure_set.pickle' % Model_Type, 'wb'))
